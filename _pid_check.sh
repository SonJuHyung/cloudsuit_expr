...
then
    ...
    ANONYMOUS_HUGE_RATIO=$(echo "${ANONYMOUS_HUGE_KB} ${VMSIZE_KB}" | awk 'BEGIN { avg=0.0;} { split($0,arr," "); avg=arr[1]/arr[3]*100.0 } END { printf("%05.3f",avg)}')

    echo "  VmSize       : ${VMSIZE}    (${VMSIZE_KB})"
    echo "  VmRss        : ${VMRSS}    (${VMRSS_KB})    [VmRssAnon + RssFile + RssShmem]"
    echo "  AnonPage     : ${ANONYMOUS_PAGE}    (${ANONYMOUS_PAGE_KB})    [${ANONYMOUS_PAGE_RATIO} %] "
    echo "  AnonHugePage : ${ANONYMOUS_HUGE}    (${ANONYMOUS_HUGE_KB})    [${ANONYMOUS_HUGE_RATIO} %]"
