...
apt-get install -y gawk bsdmainutils

DIR_CUR=$(pwd)
DIR_PERF=${DIR_CUR}/son_perf
DIR_RUN=${DIR_CUR}/son_out
DIR_SYS=${DIR_CUR}/son_sys
...
#	CONNECTION_NUM=200	
    ...
    source ./_check.sh > ${DIR_SYS}/sys_memcached_before.dat

	perf stat -e ${PMU_S} -o ${DIR_PERF}/perf_memcached.dat -a /usr/src/memcached/memcached_client//loader \
		-a /usr/src/memcached/twitter_dataset/twitter_dataset_5x \
		-s /usr/src/memcached/memcached_client/docker_servers.txt \
		-g ${GET_SET_RATIO} -T ${STATISTICS_INTERVAL} -c ${CONNECTION_NUM} -w ${THREAD_NUM} 
    # > ${DIR_RUN}/run_memcached.dat

    source ./_check.sh > ${DIR_SYS}/sys_memcached_after.dat
#	exec "$@"
fi
