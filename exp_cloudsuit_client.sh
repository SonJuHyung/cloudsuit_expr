...
case ${OP_TYPE} in 
    "data_serving") 
        ...
        sudo perf stat -e ${PMU_S} -o ${DIR_PERF}/${OP_TYPE}/${HP_TYPE}/${OP_TYPE}-${HP_TYPE}-${MFRG_TYPE}.dat -a time docker run -e FIELDLENGTH=${RCD_FLD_LENGTH} -e FIELDCOUNT=${RCD_FLD_COUNT} -e RECORDCOUNT=${RCD_COUNT} -e OPERATIONCOUNT=$OPT_COUNT -e THREADS=$THREAD_NUM --name ${NAME_CLIENT} --net ${NETWORK} ${DOCKER_IMAGE_CLIENT} ${NAME_SERVER} > ${DIR_RUN}/${OP_TYPE}/${HP_TYPE}/${OP_TYPE}-${HP_TYPE}-${MFRG_TYPE}.dat
    ...
    "web_search") 
        ...
        # start client 
        echo "  running ${OP_TYPE} experiment ..."      
        sudo perf stat -e ${PMU_S} -o ${DIR_PERF}/${OP_TYPE}/${HP_TYPE}/${OP_TYPE}-${HP_TYPE}-${MFRG_TYPE}.dat -a time docker run -it --name ${NAME_CLIENT} --net ${NETWORK} ${DOCKER_IMAGE_CLIENT} ${SERVER_ADDRESS} ${NUM_CON_CLI} ${TIME_WARMUP} ${TIME_STEADY} ${TIME_ENDING} > ${DIR_RUN}/${OP_TYPE}/${HP_TYPE}/${OP_TYPE}-${HP_TYPE}-${MFRG_TYPE}.dat 
